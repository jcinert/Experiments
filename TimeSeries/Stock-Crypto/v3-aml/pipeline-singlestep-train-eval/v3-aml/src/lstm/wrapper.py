...
def get_data(lstm_configs, data_path, use_same_train_validation=True, use_same_validation_test=True, verbose='All'):
    '''
    ...
    data_file_train = data_path / lstm_configs['data']['filename']
    ...
    if (use_same_validation_test and use_same_train_validation):
        ...
        # different data file
        data_file_test = data_path / 'data' / 'xxx.csv' # TODO FIX
...
def get_viz_data(lstm_configs, data_path, verbose='All'):
    '''
    ...
    if data_path == '':
        ...
        data_path = Path(data_path).absolute()
    data_file_viz = data_path / lstm_configs['data']['filename']
...
def get_todays_data(lstm_configs, data_path, days_shift, verbose='Summary'):
    '''
        Gets Train, Val, Test data (normalized)
        Only BTC for now
        Parameters:
...
def train_model(lstm_configs, x, y, x_val, y_val, verbose='All'):
    ...
    if verbose == 'All' or verbose == 'Summary':
        ...
        # mlflow.log_metric(f"{lstm_configs['name']}_val_acc", max(history.history['val_accuracy'])*100)
...
def viz_generate_accuracy_chart(viz_acc,add_ma=''):
    ...
    if not add_ma == '':
        fig.add_trace(go.Scatter(x=viz_acc[:,0],y=viz_acc[:,4], name=add_ma))
