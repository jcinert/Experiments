...
def merge_price_fgi_data(config, data_path, verbose=False):
    """
    ...
    if data_path == '':
        path = Path(os.getcwd())
        common_data_path = path.absolute()
    else:
        common_data_path = Path(data_path).absolute()

    # read fgi data
    fgi_path = common_data_path / config['raw_fgi_data']
    df_fgi_raw = pd.read_csv(fgi_path)

    # read raw price data
    price_path = common_data_path / config['raw_price_data']
    ...
    # save
    data_path_export = common_data_path / config['processed_marked_data']
    df_export.to_csv(data_path_export, index=False, mode='w', header=True)

    if verbose:
        print('[Data] new data shape: {}'.format(df_export.shape))
        print(f'[Data] fgi added to price data {data_path_export}')
    return 0

def generate_ta_data(config, data_path, verbose=False):
    """
    ...
    price_path = common_data_path / config['processed_marked_data']
    df_price_raw = pd.read_csv(price_path)
    ...
    if verbose:
        print('[Data] new data shape: {}'.format(df_out.shape))
        print(f'[Data] TA added to price data: {data_path_export}')
    return 0

def generate_synth_data(config, data_path, verbose=False):
    """
    ...
    # save
    data_path_export = common_data_path / config['synt_data']
    df_out_v4.to_csv(data_path_export, index=False, mode='w', header=True)

    if verbose:
        print('[Data] new data shape: {}'.format(df_out_v4.shape))
        print(f'[Data] Synthetic data generated: {data_path_export}')
    return 0
