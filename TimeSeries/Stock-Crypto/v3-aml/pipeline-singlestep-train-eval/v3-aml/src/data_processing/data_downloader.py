...
def yf_download_data(config, data_path, verbose=False):
    """
    ...
    csv_path = csv_path / config['raw_price_data']
    ...
    if verbose:
        print('[Data] - Import from yfinance completed')
        print(f'[Data] File saved to: {csv_path}')
    return 0

def fgi_download_data(config, data_path, verbose=False):
    """
    ...
    csv_path = csv_path / config['raw_fgi_data']
    df_index.to_csv(csv_path, index=False, mode='w', header=True)

    # save
    if verbose:
        print('[Data] Import from fgi completed')
        print(f'[Data] File saved to: {csv_path}')
    return 0
