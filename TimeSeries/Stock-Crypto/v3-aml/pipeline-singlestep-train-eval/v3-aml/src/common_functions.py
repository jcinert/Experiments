...
def load_config(verbose=False):
    ...
    if verbose:
        print(f'Import config lodaded: {config_path}')
    return config

def get_backfill_range(data_path, verbose=False):
    """
    returns list of dates for which forecast hasnt been done
    """
    if data_path == '':
        path = Path(os.getcwd())
        forecast_path = path.absolute()
    else:
        forecast_path = Path(data_path).absolute()
    
    forecast_path = data_path / 'forecast.csv'
...
def save_forecast(data_path, df_forecast, model, value_column='Open', verbose=False):
    """
    ...
    if data_path == '':
        ...
        csv_path = Path(data_path).absolute()
    csv_path = csv_path / 'forecast.csv'
...
def plot_prediction_tb5(model='fbp', coin='BTC-USD', day_range=30):
    ...
    path = Path(os.getcwd())
    data_path = path.absolute()
    real_data_path = data_path / 'price_data.csv'
    forecast_path = data_path  / 'forecast.csv'
...
def get_accuracy_tb(model, config, lstm_config, exclude_today=True, verbose=False):
    ...
    """
  # get forecast & price data
    forecast_path = config['aml_datapath'] + config['forecast']
    df_forecast_all = pd.read_csv(forecast_path)

    price_path = config['aml_datapath'] + config['processed_marked_data']
    df_price_data = pd.read_csv(price_path)
    ...
    for coin in df_tbl['Coin'].unique():
        ...
        # df_accuracy = df_accuracy.append([df_accuracy, pd.DataFrame({'Coin':coin, 'Accuracy':accuracy})], ignore_index=True)
        if df_accuracy.empty:
            df_accuracy = pd.DataFrame([{'Coin':coin, 'Accuracy':accuracy}])
        else:
            df_accuracy = pd.concat([df_accuracy,pd.DataFrame({'Coin':coin, 'Accuracy':accuracy})], ignore_index=True)

        if verbose:
            print('{}: TB classification accuracy over forecast window: {:,.2f}%'.format(coin,accuracy*100))
...
def get_coin_summary(df_tbl, df_accuracy, df_mape, df_accuracy_lstm):
    ...
    for idx, row in df_summary.iterrows():
        ...
        df_exp_acc = pd.concat([df_exp_acc,pd.DataFrame({'Coin':row['Coin'],'lstm-expected-acc':parameters['expected_accuracy']['acc']})], ignore_index=True)
